<script type="text/javascript">
    ...
    } else if(sessionStorage['<?php echo $SESSION."move"; ?>'] !== undefined){
       $('a.copybutton').css("display","none");
       $('a.movebutton').css("display","none");
          
            var clipboard = '<a style="pull-right" onclick="clearClipBoard()">Clear <span class="badge">'+JSON.parse(sessionStorage['<?php echo $SESSION."move"; ?>']).length+'</span>files in clipboad</a>';
                $('.controls').append(clipboard);
                 $('.filerow').each(function(){
           if($(this).find(".extension").text()== "Folder"){
               var paste = '   <a onclick=moveTo("'+$(this).find("input").attr("value")+'")><span style="font-size:12px;" class="label label-danger">Move here</span></a>';
               $(this).find(".realfiles").append(paste);
  ...
  function clearClipBoard(){
       ...
       delete sessionStorage['<?php echo $SESSION."move"; ?>'];
...
<script type="text/javascript">
   ...
   if(check==0){
       ...
       sessionStorage['<?php echo $SESSION."move"; ?>']= JSON.stringify(files);
   $('a.copybutton').css("display","none");
   $('a.movebutton').css("display","none");
   var clipboard = '<a style="pull-right" onclick="clearClipBoard()">Clear <span class="badge">'+files.length+'</span>files in clipboad</a>';
   $('.controls').append(clipboard);
   ...
   function moveTo(folder){
       //populate and submit form with values from localstorage
       var files = JSON.parse(sessionStorage['<?php echo $SESSION."move"; ?>']);
       $('form.mainform').attr("action","<?php echo $APP_URL;?>/file/moveto/"+folder);
       $('form.mainform').append('<input type="hidden" name="folder" value="'+folder+'" />');
       for(var i=0; i<files.length; i++){
           $('form.mainform').append('<input type="hidden" name=filestomove[] value="'+files[i]+'" />');
       }
       $('form.mainform').submit();
...
<div class="container drop">
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
        ...
        <div class="controls col-md-12">
            ...
            <a class="pull-left movebutton" onclick="moveFiles();"><span class="glyphicon glyphicon-arrow-right"></span><span class="glyphicon glyphicon-folder-open"></span><br/>Move</a>        
        </div>
    </div>
  ...
  <strong>Welldone!</strong> <?php echo $success; ?>
    ...
    <table class="filetable table-striped" border="0">
        ...
        ?>
        <tr class="filerow">
            <td><input class="checkme" type="checkbox" name="file[]" value="<?php echo $file['filename']; ?>"/></td>
            <td><div class="extension"><?php echo $file['fileext']; ?></div></td>
            <td class="realfiles"><a data-toggle="tooltip" data-placement="top" data-title="Click to Open(folder) or Download" href="<?php echo $APP_URL; ?>/file/<?php echo ($file['fileext']=='Folder')? 'open':'download'; ?>/<?php echo $file['filename']; ?>"><?php echo $file['filename']; ?></a></td>
            <td><?php echo date("Y-m-d h:i:sa",$file['date_mod']); ?></td>
            <td><?php echo $file['fileext']; ?> File</td>
            <td><?php echo round($file['size']/1048576, 3); ?>MB</td>
            <td><a title="Delete file" href="<?php echo $APP_URL; ?>/file/delete/<?php echo $file[filename];?>"><span class="glyphicon glyphicon-remove"></span>Delete</a> | 
                <a title="Rename File" onclick="renamePopup('<?php echo $file[filename]; ?>','<?php echo $file[fileext]; ?>')"><span class="glyphicon glyphicon-edit"></span>Rename</a> |
                <?php if($file[fileext]!= "Folder") { ?> <a title="Share File" href="<?php echo $APP_URL; ?>/share/share/<?php echo $file[filename];?>"><span class="glyphicon glyphicon-share"></span> Share</a> |<?php } ?> 
                <?php if($file[fileext]!= "Folder") { ?><a title="Download File" href="<?php echo $APP_URL; ?>/file/download/<?php echo $file[filename];?>"><span class="glyphicon glyphicon-download"></span>Download</a><?php } ?>
            </td>
        </tr>
        <?php 
        }
