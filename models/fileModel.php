...
class fileModel extends Model{
    ...
    public function copy($folder, $files){
        ...
        foreach($files as $file){
            $quotaleft = $this->getQuota();
            if(!is_dir($file[0])){
                if(filesize($file[0])< $quotaleft[2]){
                ...
                while(false !==($file2 = readdir($dir))){
                    if(($file2 != '.') && ($file2 != '..')){
                        if(is_dir($dir.DS.$file2)){
                            ...
                            if(filesize($file[0].DS.$file2) < $quotaleft[2]){
    ...
    public function moveTo($folder, $files){
        ...
        //call main file copy method
        if($this->move($folder, $realpath)){
            return true;
        }
        
    }
    
    public function move($folder, $files){
        $pathtocopy = CFactory::getSession()->getSession('cur_path').DS.$folder;
        foreach($files as $file){
             rename($file[0], $pathtocopy.DS.$file[1]()); 
    ...
    public function download($filename){
        ...
        if(!file_exists(CFactory::getSession()->getSession('cur_path').DS.$filename)){
            return false;
        } else {
            
            $file = CFactory::getSession()->getSession('cur_path').DS.$filename;
