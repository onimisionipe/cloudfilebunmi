...
class fileController extends Controller {
    ...
    public function index($message=null,$error=null){
        ...
        $this->assign('logout', 'Logout');
            ...
            $this->setTitle('Hey '. $user->getUser('fullname').' - '.'File Manager');
    ...
    public function upload(){
     ...
     public function copyto($folder){
       ...
       if(CFactory::getRequest()->post('folder')['folder']){
           ...
           if($copy === true){
               ...
               $this->error = "Quota Limit Reached";
               $this->index();
           }
       } else {
           $this->index();
     ...
     public function moveto($folder){
       ...
       if(CFactory::getRequest()->post('folder')['folder']){
           $copy = $this->loadModel('file')->moveTo(CFactory::getRequest()->post('folder')['folder'],CFactory::getRequest()->post('filestomove')['filestomove']);
           
           if($copy === true){
               $this->message = "Files has been moved successfully";
               $this->index();
           } else {
               $this->error = "Error Moving Files";
               $this->index();
           }
       } else {
           $this->index();
     ...
     public function download($filename){
        ...
        //call model to download
        $download = $this->loadModel('file')->download($filename);
        if($download === false){
            $this->error = "Error Downloading File. Please verify the file you are trying to download exists.";
        }
